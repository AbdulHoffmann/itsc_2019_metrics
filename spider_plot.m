...
function func_output = spider_plot(P, P_labels, axes_interval, axes_precision, varargin)
...
func_output = zeros(2, num_of_points, num_of_points);

% Normalized axis increment
normalized_axis_increment = 1/axes_interval;
if not(isNormalized); normalized_axis_increment = 0; end
...
for ii = 1:row_of_points
    ...
    set(f,'facealpha',.25, 'HandleVisibility','off')
    
    func_output(:,:,ii) = [x_circular(1:end-1); y_circular(1:end-1)];
end
%%% Axis Properties %%%
% Figure background
fig = gcf;
